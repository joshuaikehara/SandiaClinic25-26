...
c 13Jun07-PAS/2.60: wrapped mp tools into separate routine
...
c       Initialize MPI system
        call UTLMPINIT( node, nprocs, node0, icomm )
      else
c       Just obtain node information from preexisting MPI system
        call UTLMPINFO( node, nprocs, node0, icomm )
      endif
cmpi:end
c
...
cmpi:start Following statements enabled for mpi, else removed
     ...
     $     'MPINFO::: Global Communicator        :::'
      write(IWR,'(1x,a,i4,a)') 
     $     'MPINFO::: Global Context = ',icomm,  '      :::'
      write(IWR,'(1x,a,i4,a)') 
     $     'MPINFO::: Global Size =    ',nprocs ,'      :::'
      write(IWR,'(1x,a,i4,a)') 
     $     'MPINFO::: Global Root =    ',node0  ,'      :::'
      write(IWR,'(1x,a,i4,a)') 
     $     'MPINFO::: Global Rank =    ',node   ,'      :::'
cmpi:end
c
      RETURN
      END
...
c
      ...
      END
cmpi * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
cmpi Following routines are purely mpi-specific (deleted for serial).
cmpi * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
c
c
c >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  UTLMPINIT
c
c
      subroutine UTLMPINIT( node, nprocs, node0, icomm )
c
      IMPLICIT  NONE
      INCLUDE  'mpif.h'
c     
...
c
c Output variables
      INTEGER  node,nprocs,node0,icomm
c
c Local variables
      INTEGER  ierror
      INTEGER  istatus,getcwd,chdir
      CHARACTER  path*128
c
      if( isubcall.eq.0 ) call mpi_init( ierror )

      icomm = MPI_COMM_WORLD
      call mpi_comm_rank( icomm, node, ierror )
...
c
c
      entry UTLMPINFO( node, nprocs, node0, icomm )
c
      icomm = MPI_COMM_WORLD
      call mpi_comm_rank( icomm, node, ierror )
      call mpi_comm_size( icomm, nprocs, ierror )
...
c
      entry UTLMPFINI
c
      if( isubcall.eq.0 ) call mpi_finalize( ierror )
c
      RETURN
      END
