...
def run_robust_fix():
    ...
    # Use ATOM 2 (Si) as the "Anchor"
    anchor_idx = 1 
    anchor_atom_type = atom_types[anchor_idx]
    
    print(f"Using Anchor Atom: Index {anchor_idx+1} ({anchor_atom_type})")
    
    # 1. Find all candidate atoms in Final
    candidates = []
    for i, xyz in enumerate(final_scrambled):
        candidates.append(i)
        
    best_shift = None
    best_total_error = float('inf')

    print(f"Testing {len(candidates)} possible alignments...")

    # 2. Iterate through every possible alignment
    for cand_idx in candidates:
        shift = initial[anchor_idx] - final_scrambled[cand_idx]
        temp_final = final_scrambled + shift
        ...
        for i in range(len(initial)):
            ...
            for j in range(len(temp_final)):
                if j in used: continue
                d = np.linalg.norm(target - temp_final[j])
                if d < min_d:
                    min_d = d
                    best_j = j
            ...
            used.add(best_j)
            current_error += min_d
            if current_error > best_total_error:
                fail = True
                break
        
        if not fail and current_error < best_total_error:
            best_total_error = current_error
            best_shift = shift
            print(f"  Found better alignment! Error: {best_total_error:.4f}")

    # 3. Apply the Best Shift Found
    print(f"Best Shift Vector: {best_shift}")
    final_aligned = final_scrambled + best_shift
    
    # 4. Generate the Output Files (FIXED FORMATTING)
    with open("efinal_robust_fixed.txt", "w") as f_out, open("midpoint_robust.geom", "w") as f_geom:
        f_out.write("atom, type, position;\n")
        f_geom.write("atom, type, position;\n") # SeqQuest format header for .geom
        ...
        for i, target_pos in enumerate(initial):
            ...
            for j, candidate_pos in enumerate(final_aligned):
            ...
            # Coordinates for Input File
            x, y, z = final_aligned[best_idx]
            f_out.write(f" AT{i+1:<5} {atom_types[i]:<5} {x:14.10f} {y:14.10f} {z:14.10f}\n")
            
            # Coordinates for Midpoint Check (Formatted as SeqQuest .geom now)
            mid = (target_pos + final_aligned[best_idx]) / 2.0
            f_geom.write(f" AT{i+1:<5} {atom_types[i]:<5} {mid[0]:14.10f} {mid[1]:14.10f} {mid[2]:14.10f}\n")

    print("Done! Files generated:")
    print("  1. efinal_robust_fixed.txt (Use this for SeqQuest)")
    print("  2. midpoint_robust.geom    (SeqQuest formatted midpoint for sanity check)")
